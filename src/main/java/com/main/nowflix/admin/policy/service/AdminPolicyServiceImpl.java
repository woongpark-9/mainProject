...
public class AdminPolicyServiceImpl implements AdminPolicyService {
	...
	public int createPdf(String newpdf) {
        ...
        try {    		
            ...
            Chunk chunk = new Chunk("Policy_INFO", font); // 타이틀 객체를 생성 (타이틀의 이름을 장바구니로 하고 위에 있는 font를 사용)
            ...
            PdfPCell cell3 = new PdfPCell(new Phrase("이용약관 내용", font));
            ...
            for (int i = 0; i < list.size(); i++) {
            	AdminPolicyVO vo = list.get(i); // 레코드에 값들을 꺼내서 dto에 저장
                PdfPCell cellId = new PdfPCell(new Phrase("" + vo.getPolicy_id(), font)); // 반복문을 사용해서 상품정보를 하나씩 // 출력해서 셀에 넣고 테이블에// 저장한다.
