...
public class AdminDirectorServiceImpl implements AdminDirectorService {
	...
	public int createPdf(String newpdf) {
		...
		try {
			...
			Chunk chunk = new Chunk("Director_INFO", font); // 타이틀 객체를 생성 (타이틀의 이름을 장바구니로 하고 위에 있는 font를 사용)
			...
			PdfPCell cell1 = new PdfPCell(new Phrase("감독 아이디", font)); // 셀의 이름과 폰트를 지정해서 셀을 생성한다.
			cell1.setHorizontalAlignment(Element.ALIGN_CENTER); // 셀의 정렬방식을 지정한다. (가운데정렬)

			PdfPCell cell2 = new PdfPCell(new Phrase("감독 이름", font));
			cell2.setHorizontalAlignment(Element.ALIGN_CENTER);

			PdfPCell cell3 = new PdfPCell(new Phrase("감독 생년월일", font));
			...
			for (int i = 0; i < list.size(); i++) {
				AdminDirectorVO vo = list.get(i); // 레코드에 값들을 꺼내서 dto에 저장
				PdfPCell cellId = new PdfPCell(new Phrase("" + vo.getDirector_id(), font)); // 반복문을 사용해서 상품정보를 하나씩 //
