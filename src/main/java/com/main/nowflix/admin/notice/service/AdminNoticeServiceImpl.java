...
public class AdminNoticeServiceImpl implements AdminNoticeService {
	...
	public int createPdf(String newpdf) {
        ...
        try {    		
            ...
            Chunk chunk = new Chunk("Notice_INFO", font); // 타이틀 객체를 생성 (타이틀의 이름을 장바구니로 하고 위에 있는 font를 사용)
            ...
            for (int i = 0; i < list.size(); i++) {
            	AdminNoticeVO vo = list.get(i); // 레코드에 값들을 꺼내서 dto에 저장
                PdfPCell cellId = new PdfPCell(new Phrase("" + vo.getNotice_id(), font)); // 반복문을 사용해서 상품정보를 하나씩 // 출력해서 셀에 넣고 테이블에// 저장한다.
